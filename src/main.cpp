...
int main(int argc, char *argv[]) {
	...
	double init_cte_importance = 2000; // atof(argv[3]);
	double init_epsi_importance = 2000; // atof(argv[4]);
	double init_v_importance = 1; // atof(argv[5]);

	double init_delta_importance = 5; // atof(argv[6]);
	double init_a_importance = 10; // atof(argv[7]);

	double init_delta_gap_importance = 200; // atof(argv[8]);
	double init_a_gap_importance = 100; // atof(argv[9]);
				...
				if (s != "") {
					...
					if (event == "telemetry") {
						...
						*/
						...
						// the code below obtained from the Self-Driving Car Project Q&As | Term 2
